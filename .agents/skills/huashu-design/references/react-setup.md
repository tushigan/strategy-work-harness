...
工作包不提供在浏览器输入、保存或转发服务密钥的示例。需要真实模型能力时，先确认外部调用、资料发送和费用范围，再由项目外受控后端保管凭据并按官方接口接入；HTML 不接触服务密钥。

例如 Anthropic 的认证字段名是 `x-api-key`，不能通过替换协议名来脱敏。此处只解释接入责任，不包含后端实现，也不表示已连接服务。正式离线提案不使用这条扩展路线；演示原型默认选择 A 或 C 的模拟数据。
